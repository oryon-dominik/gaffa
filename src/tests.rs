#[cfg(test)]
mod test {
    use crate::process_manager::{ProcessError, ProcessInfo, ProcessStatus};
    use crate::constants::*;
    ...
    fn test_process_error_messages() {
        let err = ProcessError::ProcessNotFound { name: "test".to_string() };
        assert_eq!(err.to_string(), "Process 'test' not found");
        
        let err = ProcessError::ProcessAlreadyRunning { name: "web".to_string() };
        assert_eq!(err.to_string(), "Process 'web' is already running");
        
        let err = ProcessError::EmptyCommand { name: "worker".to_string() };
    ...
    fn test_parse_env_file() {
        ...
        rt.block_on(async {
            crate::parse_env_file("test_env_file", &mut env_vars).await.unwrap();
        });
    ...
    fn test_parse_env_file_edge_cases() {
        ...
        rt.block_on(async {
            crate::parse_env_file("test_env_edge", &mut env_vars).await.unwrap();
        });
